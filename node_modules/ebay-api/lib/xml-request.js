...
function buildXmlInput(options) {
  if (options.fromXml){
      return buildXmlInputXML(options)
  }
...
function buildXmlInputXML(options) {
    debug('buildXmlInputXML', options);
    ...
    var request = options.opType.toString()+'Request'
    data[request] = {};
    data[request]['$'] = {};
    data[request]['$']['xmlns'] = 'urn:ebay:apis:eBLBaseComponents';
    data[request]['RequesterCredentials'] = {};
    data[request]['RequesterCredentials']['eBayAuthToken'] = options.authToken;
    var temp1 = data[request];
    //data[request] = {temp1,params}
    data[request]['$'] = temp1['$']
    data[request]['Item'] = params['Item']
    ...
    var xmlBody = builder.buildObject(data);

    debug('XML body', xmlBody);

    var newXml = xmlBody.replace("undefined","");

    return xmlBody.replace("undefined","");
}
